...
このプランは、最新レビュー時点で**まだ残っている論点だけ**に絞った修正計画です。  
前回までの大きな改善、たとえば `generator_intimacy_md` の persistence、CoE extractor + integrator の本線化、sandbox pair state / working memory の持ち越し、deterministic ranker gate 導入などは前提として、**未完了の箇所だけ**を ticket 化しています。
...
このプランは、前回の大物修正がかなり進んだことを前提に、**残っている運用・移行・ゲートまわりの穴だけ**を ticket 化したものです。  
順番を守り、各 ticket を green にしてから次へ進めれば、「かなり良くなったが最後の詰めで崩れる」を避けやすくなります。
