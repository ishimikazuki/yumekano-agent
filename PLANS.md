...
| T5 | production chat_turn を新 core に配線 | done |
| T6 | draft/playground を stateful にし prod/draft parity を作る | done |
| T7 | generator/ranker の memory path と deterministic gates 完成 | done |
| T8 | publish/versioning を DB-backed workspace path に統一 | done |
| T9 | 最終 eval と rollout 判定 | done |
...
### 受入基準
- [x] production `chat_turn` の本線が new CoE path を使う
- [x] `turnsSinceLastUpdate: 1` の固定がなくなる
- [x] `events: new Map()`, `topics: new Map()`, zero counter placeholder が導出可能な実値に置き換わる
- [x] pair metrics が DB に保存される
- [x] trace に新しい state reasoning が残る
- [x] required workflow integration tests が green
...
### 受入基準
- [x] draft/playground が PAD を session across turns で持ち越す
- [x] draft/playground が pair metrics を持ち越す
- [x] draft/playground が sandbox working memory を持ち越す
- [x] draft の `phaseIdAfter` が固定ではない
- [x] parity test が green
...
### 受入基準
- [x] generator prompt/context が retrieved memory を使う
- [x] ranker input が memoryGrounding に必要な context を持つ
- [x] deterministic gates が code path として存在する
- [x] reject された候補は model judgement 前に落ちる
- [x] required tests が green
...
### 受入基準
- [x] active code path に in-memory drafts.ts / publish.ts 依存の publish flow が残っていない
- [x] workspace draft から publish できる
- [x] fresh DB publish smoke が green
- [x] docs に canonical publish flow が明記されている
...
### 受入基準
- [x] `npm run test`, `npm run test:db`, `npm run test:workflow`, `npm run eval:smoke`, `npm run ci:local` がすべて green
- [x] final markdown report が生成される
- [x] legacy/new emotion path の default flag が決まっている
- [x] remaining blockers がある場合は report に明示されている
- [x] "complete / not complete" を pass/fail で宣言できる
