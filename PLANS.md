...
**Status**
- [ ] **Deferred (2026-04-22)** — T-E で「押して引く」が `share_information` + phase description + authoredExamples で十分機能することを live LLM で実証。`self_disclose` enum 追加は現時点では speculative な観測性向上であり、MVP 段階では優先度低。将来「弱み見せ」を phase condition のトリガーにしたくなったときに着手する。
...
**Status**
- [ ] **Deferred (2026-04-22)** — T-E の検証で、planner が `emotionDeltaIntent.dominanceDelta` を能動的に負方向へ設定し（Turn 3-5 で確認）、D が -0.22 → -0.30 → -0.44 と自然に低下することを確認。構造的な CoE シグナル追加は speculative redundancy で、MVP では不要。planner が drift した場合に着手する。
...
### 実行したこと
- `src/mastra/scorers/question-saturation.ts` 追加（deterministic、LLM 不使用）
  - 直近 N 人目の assistant ターンが連続で質問を含むかを判定
  - 既定 windowTurns=3、saturationScore=0.3
  - `？` / `?` 両方対応
- `src/mastra/scorers/index.ts` から export 追加
- `src/mastra/agents/ranker.ts` に組み込み
  - `scoreCandidate` で Promise.all の前に synchronously 実行（LLM 不要なので並列化不要）
  - `CandidateScorerAggregate` に `questionSaturation` フィールド追加
  - `deterministicOverall` を `baseOverall * questionSaturationResult.score` で乗算ペナルティ化（saturation=1.0 で no-op、後方互換）
  - 飽和時の issue を aggregate の issues 配列にマージ

### 追加テスト
- `tests/unit/scorer-question-saturation.unit.test.ts` (7/7 pass)
- `tests/workflow/ranker-question-penalty.integration.test.ts` (7/7 pass)
- `tests/regression/ranker-baseline.regression.test.ts` (5/5 pass)

### 全体 regression
- `npm run ci:local` all green（emotion 91/91、eval:smoke 10/10、ranker 12/12）
- `npx tsc --noEmit` clean

### T-A への依存を外した理由
PLANS.md 当初は「T-A green 後」を prerequisite にしていたが、T-C は既存 `DialogueActSchema` に依存せず、生成テキスト中の「？」パターンで十分機能するため独立実装。
...
**Status**
- [ ] **Deferred (2026-04-22)** — T-E でセイラ固有の phase.description に「押して引く」方針を織り込んだことで、planner の reasoning に「弱音を見せ守りたいと思わせる」が明示的に登場（Turn 4 で確認）。キャラ横断で使う汎用 prompt への昇格は、キャラが 2 体目以降になったとき再検討する。
...
# 会話深化ワークストリーム 実行順序（実績）

1. ✅ **T-E (2026-04-21)** キャラパラメータのみで押して引く実装
2. ✅ **T-C (2026-04-22)** 質問飽和ペナルティ scorer
3. ⏸ **T-A / T-B / T-D** Deferred（T-E + T-C で MVP 水準に達したため）

## 発見・決定

- 2026-04-21: 会話深化は T0–T6 の latency 最適化とは完全に独立した別軸と判断。優先度は T3 完了後に再検討。
- 2026-04-21: 事前予測「自己開示／弱み見せは schema 追加必須」は **誤り** だった。`share_information` が自己開示の器として十分機能し、phase.description + authoredExamples + PAD 低 D baseline の 3 層で押して引くが成立する。
- 2026-04-22: live LLM 検証（3 シナリオ / 合計 13 turns）で T-E が意図通りに動作することを確認。planner が「守りたいと思わせる」を戦略として言語化し、Turn 3 で「弱み見せ + お茶誘い」が自発的に発火。
- 2026-04-22: T-C を schema 変更なしで ranker に統合。`baseOverall * questionSaturationScore` の乗算ペナルティ形式により、saturation 非発生時は完全な後方互換（1.0 no-op）。
- 2026-04-22: T-A/T-B/T-D は「将来の drift 対策」として温存。MVP では T-E + T-C の薄いレイヤーで十分。
