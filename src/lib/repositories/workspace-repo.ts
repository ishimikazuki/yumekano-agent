...
function parseSandboxEventRow(row: Record<string, unknown>): MemoryEvent {
  return MemoryEventSchema.parse({
    ...
    retrievalKeys: parseJsonSafely<unknown>(String(row.retrieval_keys_json), "workspace-repo"),
    emotionSignature: row.emotion_signature_json
      ? parseJsonSafely<unknown>(String(row.emotion_signature_json), "workspace-repo")
      : null,
    participants: parseJsonSafely<unknown>(String(row.participants_json), "workspace-repo"),
...
function parseSandboxFactRow(row: Record<string, unknown>): MemoryFact {
  return MemoryFactSchema.parse({
    ...
    object: parseJsonSafely<unknown>(String(row.object_json), "workspace-repo"),
...
export const workspaceRepo = {
  ...
  async getDraft(workspaceId: string): Promise<DraftState | null> {
    ...
    return DraftStateSchema.parse({
      identity: parseJsonSafely<unknown>(row.identity_json as string, "workspace-repo"),
      persona: normalizePersonaAuthoring(parseJsonSafely<unknown>(row.persona_json as string, "workspace-repo")),
      style: normalizeLegacyStyle(parseJsonSafely<unknown>(row.style_json as string, "workspace-repo")),
      autonomy: normalizeLegacyAutonomy(parseJsonSafely<unknown>(row.autonomy_json as string, "workspace-repo")),
      emotion: normalizeLegacyEmotion(parseJsonSafely<unknown>(row.emotion_json as string, "workspace-repo")),
      memory: parseJsonSafely<unknown>(row.memory_policy_json as string, "workspace-repo"),
      phaseGraph: parseJsonSafely<unknown>(row.phase_graph_json as string, "workspace-repo"),
  ...
  async getLatestAutosave(workspaceId: string, section: string): Promise<Autosave | null> {
    ...
    return AutosaveSchema.parse({
      ...
      workspaceId: row.workspace_id,
      section: row.section,
      data: parseJsonSafely<unknown>(row.data_json as string, "workspace-repo"),
      createdAt: row.created_at,
    });
  },
  ...
  async getTurns(sessionId: string): Promise<PlaygroundTurn[]> {
    ...
    return result.rows.map((row) =>
      PlaygroundTurnSchema.parse({
        ...
        traceJson: parseJsonSafely<unknown>(row.trace_json as string, "workspace-repo"),
        createdAt: row.created_at,
      })
    );
  },
